                ...
                @forelse ($product as $row)
                    <div class="px-4 relative card group">
                        <div class="rounded-xl overflow-hidden card-shadow relative" style="width: 287px; height: 386px">
                            ...
                            <img src="{{ $row->galleries->first()->url }}" alt=""
                                class="w-full h-full object-cover object-center" />
                        </div>
                        <h5 class="text-lg font-semibold mt-4">{{ $row->name }}</h5>
                        <span class="">Rp. {{ number_format($row->price) }}</span>
                        <br>
                        <span>{!! Str::limit($row->description, '100') !!}</span>
                        <a href="{{ route('detailProducts', $row->slug) }}" class="stretched-link">
                            <!-- fake children -->
                        </a>
                    </div>
                @empty
